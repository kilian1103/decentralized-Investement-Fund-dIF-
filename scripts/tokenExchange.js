...
async function main() {
  ...
  // address of UNISWAP decentralised exchange and token address I want to exchange to
  const factory = "0xf5D915570BC477f9B8D6C0E980aA81757A3AaC36";
  ...
  // connect contracts with ABI and wallet
  let tokenContract = new ethers.Contract(DAIaddress,tokenABI,wallet);
  let factoryContract = new ethers.Contract(factory, factoryABI, wallet);

  // ask the uniswaps contract factory for address of DAI exchange
  const exchangeAddress =  await factoryContract.getExchange(DAIaddress, {gasLimit : ethers.utils.parseEther('0.1')});
  ...
  let ExchangeContract = new ethers.Contract(exchangeAddress, exchangeABI, wallet);

  // Sell ETH for DAI
  const coinsbought = await ExchangeContract.ethToTokenSwapInput(min_tokens = 1, deadline = 10**9, 
    {value : ethers.utils.parseEther('0.1'),
    gasLimit : ethers.utils.parseEther('0.1')});
  
  console.log("DAI bought: ", coinsbought);
